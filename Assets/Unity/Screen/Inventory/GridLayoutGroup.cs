...
namespace UnityEngine.UI
{
    ...
    public class GridLayoutGroup : LayoutGroup
    {
        ...
        [SerializeField] public Vector2 m_CellSize = new Vector2(100, 100);
        ...
        private Vector2 originalcellSize = new Vector2(Screen.width/4,Screen.width/4);
    ...
    //    }
        ...
        public override void CalculateLayoutInputHorizontal()
        {
            ...
            SetLayoutInputForAxis(
                padding.horizontal + (originalcellSize.x + spacing.x) * minColumns - spacing.x,      //変更ok
                padding.horizontal + (originalcellSize.x + spacing.x) * preferredColumns - spacing.x,
                -1, 0);
        }
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            else
            {
                ...
                int cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + spacing.x + 0.001f) / (originalcellSize.x + spacing.x)));
                minRows = Mathf.CeilToInt(rectChildren.Count / (float)cellCountX);
            }

            float minSpace = padding.vertical + (originalcellSize.x + spacing.y) * minRows - spacing.y;
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            if (axis == 0)
            {
                ...
                for (int i = 0; i < rectChildren.Count; i++)
                {
                    ...
                    rect.sizeDelta = originalcellSize;
                }
                return;
            }
            ...
            {
                if (originalcellSize.x + spacing.x <= 0)
                    cellCountX = int.MaxValue;
                else
                    cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + spacing.x + 0.001f) / (originalcellSize.x + spacing.x)));

                if (originalcellSize.x + spacing.y <= 0)
                    cellCountY = int.MaxValue;
                else
                    cellCountY = Mathf.Max(1, Mathf.FloorToInt((height - padding.vertical + spacing.y + 0.001f) / (originalcellSize.x + spacing.y)));
            }
            ...
            Vector2 requiredSpace = new Vector2(
                actualCellCountX * originalcellSize.x + (actualCellCountX - 1) * spacing.x,
                actualCellCountY * originalcellSize.x + (actualCellCountY - 1) * spacing.y
            );
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (originalcellSize[0] + spacing[0]) * positionX, originalcellSize[0]);
                SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (originalcellSize[1] + spacing[1]) * positionY, originalcellSize[1]);
            }
        }
    }
}
