...
namespace GoMap {
	...
	public class GOCoordinatesRaycastForFestival : MonoBehaviour {
		...
		void Update() {
			...
			if (drag) {
				...
				Debug.Log("Call RegisterHitGameObject");
				if (GameManager.Instance.RegisterHitGameObject(Input.mousePosition))
				{
					Destroy(currentProjector);
					return;
				}
				Debug.Log("!RegisterHitGameObject");
    		}
		}
